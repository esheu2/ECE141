...
Rc=4;       %coil resistance
                        % Make sure the data file is store
...
load 'testfreqdata';    % loads the experimental frequency response data on the 
                        % mechanical plant Gp=X/Fd in the same
                        % folder
...
P_elec = 5 * (1/(Lc*s + Rc)) * 1; %electrical plant transfer function: Vs/Vc

% Designing the current controller
R2 = 5000; %value of resistor R2
R3 = 121000; %value of resistor R3
C1 = 1.3 * 10^-9; %value of capacitor C1
C2 = 1.3 * 10^-12; %value of capacitor C2

C_elec = (1/R2)*(1/s)*(R3*C1*s + 1)/(R3*C1*C2*s + C1 + C2); %electrical controller transfer function -Vc/Vs
            %specify the transfer function in terms of the resistor and
            %capacitor values given above
H_vsic = 5 * Rs ;          % transfer function: Ic/Vs
...
Gp = exp(-1.2*10^-5*s)*100/(s^2 + 100*s + 10^6)  ;   % fitted mechanical plant Gp_fit=X/F

C_mech = 10^(92/20)*(10^-7+10^-2/s)*((1+s/(6*10^3))/(1+s/(6*10^4)))^3*-5.02377*10^-1*((1+s/100)/(1+s*5.02377*10^-3)); % mechanical controller V_set/V_e
